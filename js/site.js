...
function documentReadyCallback() {
  ...
  if (localStorage.getItem("theme") === "dark") {
    ...
    document.querySelectorAll("iframe").forEach(iframe => iframe.setAttribute("theme", "dark"));
  ...
  document.addEventListener("fullscreenchange", () => {
    ...
    const isDarkTheme = document.body.getAttribute("theme") === "dark";
    const noFilter = "none";

    allIframes.forEach(iframe => {
      if (document.fullscreenElement === iframe) {
        if (isDarkTheme) {
          iframe.dataset.filterApplied = iframe.style.filter;
          iframe.style.filter = "none";
        }
      } else {
        if (iframe.style.filter == noFilter) {
          iframe.style.filter = iframe.dataset.filterApplied;
        }
      }
    });
  });  
